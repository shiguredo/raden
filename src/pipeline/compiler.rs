...
use super::cache::{PipelineCovFn, PipelineFn, SweepFn};
...
///   26 → 13 命令/4px。
/// - 2x ループアンロール: 8px/反復でループ制御オーバーヘッドを半減。
fn build_src_over(mut bcx: FunctionBuilder, ptr_type: Type) {
    let entry = bcx.create_block();
    let unroll_loop = bcx.create_block();
    let tail_quad_check = bcx.create_block();
    let tail_quad = bcx.create_block();
    let scalar_check = bcx.create_block();
    let scalar_loop = bcx.create_block();
    let exit = bcx.create_block();
    ...
    // ループカウント
    // unroll_count = count / 8 (8px = 2x I32X4 per iteration)
    // has_tail_quad = (count / 4) % 2 (残り 4px チャンクの有無)
    // remainder = count % 4 (残り 0-3px)
    let simd_count = bcx.ins().ushr_imm(count, 2);
    let unroll_count = bcx.ins().ushr_imm(simd_count, 1);
    let has_tail_quad = bcx.ins().band_imm(simd_count, 1);
    let remainder = bcx.ins().band_imm(count, 3);
    let zero = bcx.ins().iconst(ptr_type, 0);

    let has_unroll = bcx.ins().icmp(IntCC::NotEqual, unroll_count, zero);
    bcx.ins().brif(
        has_unroll,
        unroll_loop,
        &block_args(&[dst, zero]),
    ...
    let thirty_two = bcx.ins().iconst(ptr_type, 32);
    ...
    // === tail_quad_check ブロック ===
    bcx.append_block_param(tail_quad_check, ptr_type);
    bcx.switch_to_block(tail_quad_check);
    let current_dst = bcx.block_params(tail_quad_check)[0];
    let has_quad = bcx.ins().icmp(IntCC::NotEqual, has_tail_quad, zero);
    bcx.ins().brif(
        has_quad,
        tail_quad,
    ...
    // === tail_quad ブロック (残り 4px) ===
    bcx.append_block_param(tail_quad, ptr_type);
    bcx.switch_to_block(tail_quad);
    let current_dst = bcx.block_params(tail_quad)[0];
