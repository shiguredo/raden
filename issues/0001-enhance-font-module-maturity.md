...
font モジュールを正式リリース可能な品質にする。raden は Blend2D 互換の 2D レンダリングライブラリであり、本 issue は `docs/BLEND2D.md` のフォント API 一覧 (L402-457) およびストロークテキスト操作 (L175-176) を完了条件の基準として、concrete issue 0021-0027 と tracked items の進捗を束ねるトラッカーとして機能する。
...
- 公開 API:
  ...
  - `Font`: `from_face()`, `size()`, `scale()`, `map_char_to_glyph()`, `glyph_advance()`, `append_glyph_outline()`, `measure_text()`, `ascent()`, `descent()`
  - `FontError`
  - `TextMetrics`
...
- `tests/test_font.rs` に単体テスト 9 件: `font_face_metrics` / `font_char_to_glyph` / `font_glyph_outline_to_path` / `font_space_glyph_has_no_outline` / `font_fill_text_integration` / `font_measure_text_empty` / `font_measure_text_single_char` / `font_measure_text_newline_no_panic` / `font_glyph_advance_out_of_range_is_zero`
- 全テストが macOS の Arial.ttf に依存し、Arial 不在時はスキップ。CI の Ubuntu / Windows ジョブでは全テストがスキップ
- PBT (`pbt/tests/prop_font/main.rs`) は 0022 で新規作成済み
- fuzzing 基盤 (`fuzz/Cargo.toml`) は未作成 (`fuzz/.gitkeep` のみ。`Cargo.toml` で `exclude = ["fuzz"]`)

## Blend2D との対応

raden の存在意義は Blend2D 互換のため、`docs/BLEND2D.md` のフォント API (L402-457) とストロークテキスト操作 (L175-176) を本 issue の基準とする。本 issue を closed にする時点で対象行が「実装済み」「別 issue で対応」「対象外 (理由)」のいずれかに確定していること (完了条件「BLEND2D.md 同期」参照)。

### Blend2D との対応表: concrete issue 0021-0027 で解消する行
...
| L420 | `diag_flags()` | 対象外 | raden では `diag_flags` に相当する診断フラグを公開しない (`outline_type` で十分) |
| L451 | `get_glyph_outlines(...)` | 0027 | `Font::append_glyph_outline()` が CFF / CFF2 に対応 |

### スコープ外行の判定

上表に含まれない L402-457 / L175-176 の全行は close 前に「別 issue で対応」または「対象外 (理由)」に確定させる。1 行に複数 API が混在する行（L175-176 の `stroke_utf8_text` / `stroke_utf16_text` / `stroke_utf32_text` 等）は、実装済み API と対象外 API を分割するか、対象外 API に注記（例: 「対象外 (UTF-16 / UTF-32 テキスト経路は Rust `&str` UTF-8 入力に統一)」）を付けて確定させる。

代表例として、`BLFontFace::data()` / `weight()` / `family_name()` / `face()` / `set_size()` 以外の variation 系 / 縦書きメトリクス / underline / strikethrough / `BLFontMetrics` の `x_min..y_max` / `BLFontDesignMetrics` の `lowest_ppem` / `h_min_lsb` / `glyph_bounding_box` / `BLFontFace::create_from_file` / テーブル直接アクセス / 一括 API / `Font::matrix()` (raden は等方スケールのみ。回転 / スキューは `Context::transform` で処理) / `Font::scale()` (raden 独自) / UTF-16 / UTF-32 / LATIN1 / WCHAR テキスト経路 (raden は Rust `&str` UTF-8 入力に統一) / `stroke_glyph_run` などがある。判定は本 issue close PR でまとめて確定 (`docs/BLEND2D.md` を直接編集)。
...
- font モジュールの公開 API は本 issue close までは **未安定** とみなし、0021-0027 で破壊的変更を許容する
...
本 issue で追跡するが、concrete issue ファイルとして未作成の項目。番号は `SEQUENCE` に従う (現状 SEQUENCE = 28)。
...
- **含まれない**: 0021-0027 が追加するテーブル (OS/2 / kern / GSUB / GPOS / CFF / CFF2) の PBT、cmap Format 0/2/6 等の他フォーマット、パニック耐性 (fuzzing 基盤 tracked の役割。`pbt/README.md` 方針)
- **完了基準**: 上記関数全てに不変条件 PBT があり、`cargo test -p pbt --test prop_font` が CI でパス
- **担当**: 0021 並行着手可能ブロック内で先着が作成する。個別 concrete issue 化は行わず、本 tracked item を 0021 / 0022 / 0023 のいずれかの PR で closed にする
...
### 並行着手可能ブロック (0021 / 0023)

0021 / 0023 は相互に依存しないため並行着手可能。0022 は closed。
...
0021-0027 と前提 concrete issue の全 concrete issue が close、CFF / CFF2 判定が確定、raden 全体 SemVer / 安定化方針別 issue が起票済の後、`docs/BLEND2D.md` のフォント API 一覧 (L402-457) およびストロークテキスト操作 (L175-176) の全行を確定状態 (`実装済み` / `別 issue` / `対象外`) に揃える PR で本 issue を closed にする。
...
(本 issue を closed にした時に追記する。完了した全 concrete issue の番号と完了日、Tracked items の最終状態、当初予定との差異、CFF / CFF2 判定結果、`docs/BLEND2D.md:402-457` および L175-176 の最終状態サマリを含める)

## 完了条件

以下すべてを満たした時点で本 issue を closed にする。
...
- `docs/BLEND2D.md:402-457` および L175-176 の全行が以下のいずれかに確定:
  ...
  - `対象外 (理由)`
- `未実装` / `差異あり` / `一致` / `raden 独自` など、許容状態 (`実装済み` / `別 issue で対応 (issue 番号)` / `対象外 (理由)`) 以外の行が残っていない。検証コマンド例: `sed -n '405,457p;175,176p' docs/BLEND2D.md | grep -nE '^\|.*\|(未実装|差異あり|一致|raden 独自)'` がマッチ 0 件（テーブル本体の行のみを対象とする）
...
- fuzzing で font テーブルパーサのクラッシュ耐性が検証されている。本 issue を closed にする直前の直近 1 回の 24 時間連続実行で新規クラッシュ 0 件 (修正が発生したら修正反映後の連続実行が新規クラッシュ 0 件で 24 時間に到達すればよい。複数回修正が必要な場合の総実行時間は本完了条件のスコープ外)
