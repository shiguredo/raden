...
fn bench_stroke_poly(c: &mut Criterion, num_vertices: usize, label: &str) {
    ...
    for &size in &RECT_SIZES {
        ...
        group.bench_function(BenchmarkId::new("SrcOver", format!("{size}x{size}")), |b| {
            b.iter(|| {
                ...
                ctx.set_stroke_width(STROKE_WIDTH);
                for poly in &polys {
                    ctx.set_stroke_style(poly.color);
...
fn bench_stroke_triangle(c: &mut Criterion) {
    bench_stroke_poly(c, 3, "StrokeTriangle");
}

fn bench_stroke_poly10(c: &mut Criterion) {
    bench_stroke_poly(c, 10, "StrokePoly10");
}

fn bench_stroke_poly20(c: &mut Criterion) {
    bench_stroke_poly(c, 20, "StrokePoly20");
}

fn bench_stroke_poly40(c: &mut Criterion) {
    bench_stroke_poly(c, 40, "StrokePoly40");
}

criterion_group!(
    benches,
    bench_stroke_triangle,
    bench_stroke_poly10,
    bench_stroke_poly20,
    bench_stroke_poly40,
);
